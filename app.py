...
def retrieve_semantic_recommendations(
    ...
    similar_docs = vector_db.similarity_search(query, k=50)  # FIX: changed query_text to query
    matched_isbn_list = [
    ...
    if category and category != "All":
        books_recs = books_recs[books_recs["simple_categories"] == category].head(
            final_top_k)  # FIX: Correct list filtering
    ...
    if tone:
        ...
        if tone in emotion_mapping:
            books_recs = books_recs.sort_values(by=emotion_mapping[tone], ascending=False)  # FIX: Removed inplace=True
